...
class ToolInput(BaseModel):
    ...
    tool_name: str = Field(description="The tool_name selected based on subtask requirements")
    tool_args: dict[str, Any] = Field(description="Parameters for tool execution following the tool's input_schema")
...
class ToolSelectorV2Output(BaseModel):
    selected_tool: ToolInput | None = Field(description="The tool selected to execute the given subtask. None if no matching tool found.")
    failure_reason: str | None = Field(description="Reason why no tool was selected. None if a tool was successfully selected.")


class ToolSelectorV2(LLMCallV2[ToolSelectorV2Input, ToolSelectorV2Output]):
    def __init__(self, *, client):
        model = "gpt-oss:20b"
        system_prompt_path = "prompts/v2/tool_selector_system_prompt_v2.jinja2"
        user_prompt_path = "prompts/v2/tool_selector_user_prompt_v2.jinja2"
        options = {
            "temperature": 0.2,
        }

...
class TestSearchEngineCollection:
    ...
    def test_add_single_document(self, collection):
        ...
        assert collection._bm25 is not None

    def test_add_multiple_documents(self, collection):
    ...
    def test_add_documents_with_metadata(self, collection):
    ...
    def test_add_documents_without_metadata(self, collection):
    ...
    def test_add_duplicate_id_raises_error(
        self, collection, first_ids, first_documents, second_ids, second_documents
    ):
    ...
    def test_query(self, collection):
        ...
        assert result.ids[0][1] == "doc2"

    def test_add_duplicate_id_in_batch_raises_error(self, collection):
    ...
    def test_tokenize(self, collection, text, expected_tokens):
        """Test the tokenization method with various inputs."""
        tokens = collection._tokenize(text)
        assert tokens == expected_tokens

    def test_bm25_index_is_built(self, collection):
    ...
    def test_incremental_add_rebuilds_index(self, collection):
        """Test that adding documents incrementally rebuilds the BM25 index."""
        collection.add(
            ids=["doc1"],
            documents=["First document"],
        )
        first_bm25 = collection._bm25
        ...
        assert collection._bm25 is not first_bm25
